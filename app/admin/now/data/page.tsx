...
export default async function NavientyNowDataIndexPage({
  ...
  const [{ access }, schemaCatalog, params] = await Promise.all([
    requireNowAdmin(),
    getNowSchemaCatalog(),
    searchParams,
  ]);
  ...
  const showUnregistered =
    ...
    (!activeGroup || activeGroup === 'system') &&
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="border-b border-slate-100 bg-gradient-to-l from-violet-50 via-white to-white p-6 sm:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h2 className="text-2xl font-black sm:text-3xl">كل بيانات Navienty Now</h2>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                الواجهة تقارن Admin Registry بالـschema الحقيقي في قاعدة البيانات. الجداول
                التشغيلية الحساسة تظهر للقراءة والمراجعة فقط، بينما جداول المحتوى والإعدادات تسمح
                بعمليات CRUD حسب صلاحيتك.
              </p>
            </div>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="rounded-xl bg-slate-100 px-4 py-2 text-xs font-black text-slate-600">
                Schema coverage: {NOW_TABLES.length}/{schemaCatalog.length}
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="font-black text-amber-950">
                اكتشفنا {unregisteredTables.length} جدول جديد غير مسجل في Admin Registry
              </h3>
              <p className="mt-1 text-sm leading-6 text-amber-800">
                الجداول دي لن تختفي: تظهر تلقائيًا هنا للـmanage_settings كقراءة فقط لحين تحديد
                صلاحياتها وطريقة التعديل الآمنة.
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                {unregisteredTables.map((table) => (
                  <Link
                    key={table.table_name}
                    href={`/admin/now/data/${table.table_name}`}
                    className="rounded-xl border border-amber-300 bg-white px-3 py-2 font-mono text-xs font-bold text-amber-900 hover:bg-amber-100"
                  >
                    now.{table.table_name}
                  </Link>
                ))}
              </div>
            </div>
        ...
        {visibleTables.map((definition) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-lg font-black text-slate-950">{definition.labelAr}</p>
            ...
            <p className="mt-4 text-sm leading-6 text-slate-600">{definition.descriptionAr}</p>
      ...
      {visibleTables.length === 0 && !showUnregistered ? (
