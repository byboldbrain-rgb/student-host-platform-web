      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                تشغيل · كتالوج · تسويق · دفع · امتثال · إعدادات
              </p>
            </div>
          ...
          <nav className="mt-4 flex flex-wrap items-center gap-2">
            ...
            {access.permissions.view_orders ? (
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 hover:border-violet-300 hover:text-violet-700"
              >
            ...
            {access.permissions.manage_orders || access.permissions.manage_settings ? (
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-black text-amber-800 hover:bg-amber-100"
              >
                <ShieldCheck size={15} />
                المراجعات
              </Link>
            ...
            {access.permissions.manage_catalog ? (
              <>
                <Link
                  href="/admin/now/data?group=catalog"
                  className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 hover:border-violet-300 hover:text-violet-700"
                >
                  <Store size={15} />
                  المتاجر والكتالوج
                ...
                <Link
                  href="/admin/now/data?group=marketing"
                  className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 hover:border-violet-300 hover:text-violet-700"
                >
                  <Megaphone size={15} />
                  التسويق والعروض
            ...
            {access.permissions.manage_finance ? (
              <Link
                href="/admin/now/data?group=payments"
                className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 hover:border-violet-300 hover:text-violet-700"
              >
                <Banknote size={15} />
                الدفع
            ...
            <Link
              href="/admin/now/data"
              className="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-3 py-2 text-xs font-black text-white hover:bg-violet-700"
            >
              <Database size={15} />
              كل بيانات Now
            </Link>
        ...
        <div className="border-t border-slate-100 bg-slate-50">
          <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-2 px-4 py-2 text-[11px] font-bold text-slate-500 sm:px-6 lg:px-8">
            <span>
              الصلاحية: <strong className="text-slate-800">{access.now_role}</strong>
            </span>
