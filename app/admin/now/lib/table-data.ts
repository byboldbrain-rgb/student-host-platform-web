...
const RELATION_SPECS: Record<string, Record<string, RelationSpec>> = {
  ...
  payment_method_accounts: {
    payment_method_id: { table: 'payment_methods', value: 'id', labels: ['name_ar', 'name_en', 'code'] },
  },
};
...
export async function getNowTableColumns(tableName: string) {
  ...
  const { data, error } = await resolved.supabase
    .schema('now')
    .rpc('get_admin_table_columns', { p_table_name: tableName });
...
export async function getNowRelationOptions(tableName: string): Promise<RelationOptionsMap> {
  ...
  const entries = await Promise.all(
    Object.entries(specs).map(async ([column, spec]) => {
      ...
      const options = ((data ?? []) as NowAdminRow[])
        .map((row) => ({
          value: String(row[spec.value] ?? ''),
          label: relationLabel(row, spec.labels) || String(row[spec.value] ?? ''),
        }))
...
export async function getAdminTableRows(input: {
  ...
  let query = admin
    ...
    .select('*', { count: 'exact' })
    .order(definition.orderBy, { ascending: definition.orderAscending ?? false })
    .range(from, to);
