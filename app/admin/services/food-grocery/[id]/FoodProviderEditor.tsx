...
export default function FoodProviderEditor({
  provider,
  menuCategories,
  menuItems,
  deliveryAreas,
  ...
  const inferredProviderKind: ProviderFormState['provider_kind'] = existingSubcategories.some(
    (subcategory) => subcategory.slug === 'groceries'
  )
    ? 'supermarket'
    : 'restaurant'

  const [providerForm, setProviderForm] = useState<ProviderFormState>({
    category_id: foodGroceryCategory
      ? String(foodGroceryCategory.id)
      : String(provider.category_id || ''),
    provider_kind: inferredProviderKind,
  ...
  const isRestaurant = providerForm.provider_kind === 'restaurant'

  const steps = useMemo<DisplayStep[]>(() => {
    const base: Array<DisplayStep & { hidden?: boolean }> = [
      {
        id: 1,
        key: 'info',
        title: 'Basic Info',
    ...
    return base.filter((step) => !step.hidden)
  }, [isRestaurant])
  ...
  const validateAll = () => {
    ...
    if (!providerForm.provider_kind) {
      return 'Please select provider type.'
    }
  ...
  const handleSaveAll = async () => {
    try {
      ...
      const providerRes = await fetch(`/api/admin/food-grocery/providers/${provider.id}`, {
        method: 'PATCH',
      ...
      if (isRestaurant) {
        ...
        const menuRes = await fetch(`/api/admin/food-grocery/providers/${provider.id}/menu`, {
          method: 'PUT',
          body: menuFormData,
        })
        ...
        const zonesRes = await fetch(`/api/admin/food-grocery/providers/${provider.id}/zones`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
      ...
      const assetsRes = await fetch(`/api/admin/food-grocery/providers/${provider.id}/assets`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const renderStepContent = () => {
    ...
    if (current.key === 'menu' && isRestaurant) {
      return (
        <MenuBuilder
