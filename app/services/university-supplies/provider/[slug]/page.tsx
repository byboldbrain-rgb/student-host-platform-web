...
import RestaurantDetailsClient from '../../../restaurants/[slug]/RestaurantDetailsClient'
...
const TRANSLATIONS = {
  ...
  ar: {
    ...
    add: 'إضافة',
    noMenu: 'لا توجد عناصر متاحة حالياً.',
    ...
    languageLabel: 'اللغة',
    sizes: 'الخيارات',
    chooseSize: 'اختر الخيار',
    selectedSize: 'الخيار المختار',
...
export default async function UniversitySuppliesProviderPage({
  params,
  searchParams,
}: {
  ...
  const [{ data: categories }, { data: providerBySlug }] = await Promise.all([
    supabase
  ...
  let provider = providerBySlug as Provider | null

  if (!provider && /^\d+$/.test(slug)) {
    ...
    provider = providerById as Provider | null
  }

  if (!provider) notFound()

  if (String(provider.category_id) !== String(universitySuppliesCategoryId)) {
    notFound()
  }
  ...
  const providerCityId = provider.city_id ? String(provider.city_id) : null
