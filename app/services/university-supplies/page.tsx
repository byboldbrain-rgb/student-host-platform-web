...
const TRANSLATIONS = {
  en: {
    ...
    searchIntroTitle: 'Browse university supplies',
    searchIntroText:
      'Choose a subcategory to continue to the university supplies search page.',
  },
  ar: {
    homes: 'المنازل',
    services: 'الخدمات',
    ...
    searchIntroTitle: 'تصفح المستلزمات الجامعية',
    searchIntroText:
      'اختر تصنيفًا فرعيًا للانتقال إلى صفحة البحث الخاصة بالمستلزمات الجامعية.',
  },
} as const

const SUBCATEGORY_ICON_MAP = [
  { keywords: ['notebook', 'notebooks', 'دفاتر', 'كشاكيل', 'كراسة', 'كراسات'], url: 'https://i.ibb.co/4nT2N0y/notebook.png' },
  { keywords: ['pen', 'pens', 'قلم', 'أقلام'], url: 'https://i.ibb.co/qJX2x8W/pen.png' },
  { keywords: ['pencil', 'pencils', 'قلم رصاص', 'رصاص'], url: 'https://i.ibb.co/mC2D0J0/pencil.png' },
  { keywords: ['bag', 'bags', 'backpack', 'شنطة', 'شنط', 'حقيبة'], url: 'https://i.ibb.co/H4JcC7N/bag.png' },
  { keywords: ['book', 'books', 'كتاب', 'كتب'], url: 'https://i.ibb.co/JtWJ0w7/book.png' },
  { keywords: ['printing', 'print', 'printer', 'طباعة', 'مطبعة'], url: 'https://i.ibb.co/f1Jr8kz/printing.png' },
  { keywords: ['stationery', 'ادوات مكتبية', 'أدوات مكتبية', 'مكتبية'], url: 'https://i.ibb.co/6J2YX6Q/stationery.png' },
  { keywords: ['laptop', 'laptops', 'لاب', 'لابتوب'], url: 'https://i.ibb.co/2k5YFfV/laptop.png' },
  { keywords: ['calculator', 'calculators', 'آلة حاسبة', 'حاسبة'], url: 'https://i.ibb.co/7b9zCkR/calculator.png' },
] as const
...
function getSubcategoryVisual(subcategory: ServiceSubcategory) {
  ...
  return {
    type: 'image' as const,
    value: 'https://i.ibb.co/6J2YX6Q/stationery.png',
  }
}

export default async function UniversitySuppliesPage({
  searchParams,
}: {
  searchParams: Promise<SearchParams>
}) {
  const { lang, currency, city_id, university_id, require_search } = await searchParams
  ...
  const universitySuppliesCategoryId = universitySuppliesCategory
    ? String(universitySuppliesCategory.id)
    : UNIVERSITY_SUPPLIES_CATEGORY_ID

  const universitySuppliesSubcategories = serviceSubcategories.filter(
    (subcategory) => String(subcategory.category_id) === universitySuppliesCategoryId
  )
  ...
  const buildUniversitySuppliesSubcategoryLink = (subcategory?: ServiceSubcategory) => {
    const params = new URLSearchParams()
    params.set('lang', selectedLanguage)
    params.set('currency', selectedCurrency)

    if (subcategory?.id) {
      params.set('subcategory_id', String(subcategory.id))
    }

    if (city_id) params.set('city_id', city_id)
    if (university_id) params.set('university_id', university_id)
    if (require_search) params.set('require_search', require_search)

    return `/services/university-supplies/search?${params.toString()}`
  }
  ...
  const renderSubcategoryIconCard = (subcategory: ServiceSubcategory) => {
    ...
    return (
      <Link
        ...
        href={buildUniversitySuppliesSubcategoryLink(subcategory)}
